...
namespace {
    ...
    bool validate_content_length_transfer_encoding(const NHttp::THttpHeaders& headers, std::string_view body) {
        ...
        if (content_length_value_opt.has_value() && transfer_encoding_value_opt.has_value()) {
            return false;
        }
        ...
        } else if (transfer_encoding_value_opt.has_value()) {
            auto transfer_encoding_values = headers.get_values("Transfer-Encoding");

            size_t counter_chunked = 0;
            constexpr std::string_view chunked = "chunked";


            for (std::string_view value : transfer_encoding_values) {
                for (std::string_view current_value : split(value)) {
                    if (current_value.empty()) {
                        return false;
                    }

                    if (NHttp::is_equal_case_insensitive(current_value, chunked)) {
                        ++counter_chunked;
                    }
                }
            }

            if (counter_chunked != 1) {
                return false;
            }

            if (!NHttp::is_equal_case_insensitive(
                split(transfer_encoding_values.back()).back(), 
                chunked
            )) {
                return false;
            }
...
namespace NHttp::NModel {
    ...
    bool validate_version(const THttpVersion& version) {
    ...
    bool validate_method(EHttpRequestMethod method) {
        return method != EHttpRequestMethod::NOT_SET;
    }

    bool validate_target(std::string_view target) {
    ...
    bool validate_status(EHttpResponseStatus status) {
        return status != EHttpResponseStatus::NOT_SET;
    }
    ...
    bool validate_headers_response(const THttpHeaders& headers, std::string_view body, EHttpResponseStatus status) {
        ...
        {
            if (status != EHttpResponseStatus::SWITCHING_PROTOCOLS) {
...

        {
            ...
            if (status == EHttpResponseStatus::SWITCHING_PROTOCOLS) {
            ...
            } else if (status == EHttpResponseStatus::NO_CONTENT) {
                if (transfer_encoding_value_opt.has_value()) {
                    return false;
                }
                if (content_length_value_opt.has_value()) {
                    if (content_length_value_opt.value() != std::string("0")) {
                        return false;
                    }
                }
                ...
                if (location_values.front().empty()) {
                    return false;
                }
